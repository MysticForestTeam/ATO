...
class Carousel extends React.Component {
    ...
    componentWillMount() {
        ...
        this.setState({activeDots: List.of(...this.props.images.map((image, index) => ({id: index, state: index === 0 ? true : false}))).toJS()})
    }
    ...
    moveImages = (index) => {
        this.setState({
            translateX: this.state.translateX + ((232 + 20) * ((index === this.state.activeDots.length ? 0 : index) - this.state.activeSlide) * -1),
            activeSlide: index === this.state.activeDots.length ? 0 : index
        })

        const activeDots = List.of(...this.state.activeDots.map(dot => {
            return {id: dot.id, state: dot.id === (index === this.state.activeDots.length ? 0 : index) ? true : false}
        }))
